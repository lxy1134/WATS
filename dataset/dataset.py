import torch
from dgl import AddSelfLoop, DGLGraph, to_homogeneous
from dgl.data import QuestionsDataset, TolokersDataset, MinesweeperDataset, RomanEmpireDataset, CiteseerGraphDataset, CoraGraphDataset, PubmedGraphDataset, RedditDataset, CoraFullDataset, AmazonCoBuyComputerDataset, AmazonCoBuyPhotoDataset, CoauthorCSDataset, CoauthorPhysicsDataset
...
class Dataset:
    ...
    def _prepare_data(self, data):
        if self.ds_name in ["cora", "citeseer", "pubmed", "reddit", "questions"]:
            g = data[0]
            ...
            if self.ds_name == "questions" or self.ds_name == "roman":
                g = to_homogeneous(g)

        elif self.ds_name  in ["cora-full", "computers", "photo", "roman", "tolokers"]:
            
            g = data[0]
            # if self.ds_name == "squirrel" or self.ds_name == "roman":
            #     g = to_homogeneous(g)
            # print(f"Debug: Dataset {self.ds_name} ndata keys AFTER to_homogeneous: {list(g.ndata.keys())}")
            features = g.ndata["feat"]
            labels = g.ndata["label"]
            if self.ds_name == "tolokers" or self.ds_name == "roman":
                g = to_homogeneous(g)
                # print(g)
            ...
            # if self.ds_name == "roman":
            #     if features.dtype != torch.float32:
            #         features = features.float()
            #     min_vals = features.min(dim=0, keepdim=True)[0]
            #     max_vals = features.max(dim=0, keepdim=True)[0]
            #     # Add a small epsilon to avoid division by zero if all feature values are the same
    ...
    def _split_data(self, data):
        ...
        if self.ds_name in ["cora", "citeseer", "pubmed", "reddit", "questions"]:
            ...
            split_res = np.split(idx, [int(0.15 * len(idx)), int(0.20 * len(idx))])
            ...
            split_res = np.split(idx, [int(0.15 * len(idx)), int(0.20 * len(idx))])
            train_idx, val_idx, test_idx = split_res[0], split_res[1], split_res[2]
            print(f"Debug: Dataset {self.ds_name} train_idx: {len(train_idx)}, val_idx: {len(val_idx)}, test_idx: {len(test_idx)}")
 
            for _ in range(self.n_runs):
                train_idxs.append(np.array(train_idx))
                val_idxs.append(np.array(val_idx))
                test_idxs.append(np.array(test_idx))
            print(f"Debug: Dataset {self.ds_name} train_idx: {len(train_idxs)}, val_idx: {len(val_idxs)}, test_idx: {len(test_idxs)}")
...
def load_dataset(ds_name):
    if ds_name== "cora":
        ...
        data = SquirrelDataset(transform=AddSelfLoop())
    elif ds_name == "tolokers":
        data = TolokersDataset(transform=AddSelfLoop())
    elif ds_name == "questions":
        data = QuestionsDataset(transform=AddSelfLoop())
    elif ds_name == "roman":
        data = RomanEmpireDataset(transform=AddSelfLoop())
