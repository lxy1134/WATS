...
class Solver:
    ...
    #     # 假设 GNN 模型在 self.model.layers 中定义了网络层
    #     # (需要确保 self.model 的结构与此假设一致)
    ...
    #         # 根据层名分组参数
    #         if 'layers.0' in name:
    #             input_layer_params.append(param)
    #         else:
    #             other_layer_params.append(param)

    #     # 从配置文件读取衰减值，或者直接在这里定义
    #     # 这里假设配置文件中有 scheduler_params
    #     # e.g., conf.train['scheduler_params'] = {'input_decay': 1e-5, 'other_decay': 5e-4}
    #     # 如果配置中没有，则使用默认值
    #     scheduler_params = self.conf.train.get('scheduler_params', {})
    #     input_decay = scheduler_params.get('input_decay', 1e-5)
    #     other_decay = scheduler_params.get('other_decay', 1e-5)

    #     param_groups = [
    #     {'params': input_layer_params, 'weight_decay': input_decay},
    #     {'params': other_layer_params, 'weight_decay': other_decay}
    #     ]
    
    #     # 使用新的参数组来创建优化器
    #     # 注意：此时不要在 Adam() 中再传入全局的 weight_decay 参数
    #     self.optimizer = torch.optim.Adam(param_groups, lr=self.conf.train["lr"])
    #     # self.optimizer = torch.optim.Adam(self.model.parameters(), lr=self.conf.train["lr"], weight_decay=self.conf.train["weight_decay"])
    #     self.best_val_acc = -1
    #     self.recorder = Recorder(self.conf.train['patience'])
    #     self.weights = None
    ...
    def _save_nodewise_results(self, mode):
        if mode == 'val':
            idx, model = self.val_idx, self.model
        elif mode == 'test':
            idx, model = self.test_idx, self.model
        elif mode == 'calibration':
            idx, model = self.test_idx, self.calibrated_model
        ...
        if mode in ['test', 'calibration']:
            ...
            confidence  = prob.max(axis=1)[idx]          # ← 只取 idx 子集
            pred        = prob.argmax(axis=1)[idx]       # ← 同上
